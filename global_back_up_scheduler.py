...
# ====================================================
# 📂 [⚙️ 🌐 해외 뉴스 전용 자동 백업 및 메모리 디톡스 정책 상수 설정]
# 국내 back_up_scheduler.py와 완전히 분리된 별도 모듈. 백업 폴더(news_back_up)는
# 공유하되, 파일명에 global_ 접두어를 붙여 국내 백업본과 절대 충돌하지 않게 한다.
# ====================================================
...
async def daily_backup_and_cleanup_scheduler(db_lock, file_paths, memory_caches, load_content_func, print_status_func):
    """
    매일 00:00:00 KST를 계산하여 대기 후, 자정이 되면 🌐 해외 뉴스 캐시에 대해
    백업 및 디스크 파일 초기화, 메모리 디톡스를 수행합니다.
    """
    logger.info("⏰ [🌐 해외] 일별 자동 백업 및 청소 스케줄러가 독립 모듈에서 가동되었습니다.")
    ...
    while True:
        ...
        try:
            ...
            logger.error(f"❌ [🌐 해외 백업 에러] 자정 백업 및 정리 작업 중 치명적 오류 발생: {e}")
...
def execute_midnight_processing(db_lock, file_paths, memory_caches, load_content_func, print_status_func):
    """
    스레드 락(db_lock)을 획득하여 안전하게 🌐 해외 뉴스 디스크 파일을 분리 백업 및 초기화하고,
    ...
    with db_lock:
        logger.info(f"🔒 [🌐 해외 자정 백업 시작] 공유 리소스 동기화 잠금을 획득했습니다.")
        ...
        for key in ["stream", "filtered", "blacklisted"]:
            ...
            if file_data:
                ...
                # 3. 'news_back_up' 폴더에 분리 저장 (국내 백업본과 파일명이 겹치지 않도록 global_ 접두어 사용)
                if yesterday_data:
                    backup_filename = BACKUP_DIR / f"global_news_list_{key}_backup_{date_str}.json"
        ...
        logger.info(f"🧹 [🌐 해외] 자정 가상 메모리(RAM) 달력 일자 기준 청소 완료 (오늘: {today_date} / 어제: {yesterday_date} 데이터만 보존).")
        print_status_func()

    logger.info(f"🔓 [🌐 해외 자정 백업 종료] 공유 리소스 잠금을 해제했습니다.\n")


# 수동 재실행용: python global_back_up_scheduler.py
# (반드시 main_fastapi.py가 있는 프로젝트 루트에서 실행해야 함. 서버는 켜지지 않고,
#  main_fastapi 임포트 시 적재되는 실제 해외 캐시/db_lock을 그대로 사용해 자정 처리를 즉시 1회 실행한다.)
if __name__ == "__main__":
    import main_fastapi as mf

    print("🛠️ [수동 실행] 🌐 해외 뉴스 자정 백업/디톡스 처리를 즉시 1회 실행합니다.")
    execute_midnight_processing(
        db_lock=mf.db_lock,
        file_paths=mf.FILE_PATHS_CONFIG_GLOBAL,
        memory_caches=mf.MEMORY_CACHES_CONFIG_GLOBAL,
        load_content_func=mf.load_file_entire_content,
        print_status_func=mf.print_global_resource_status,
    )
    print("✅ [수동 실행] 🌐 해외 뉴스 자정 백업/디톡스 처리가 완료됐습니다.")